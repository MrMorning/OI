...
const int kDisMax = kNMax*7;

int dis[kDisMax], nDis;
int n, g, r, p, q, L[kNMax], t[kNMax], s[kNMax];
ll ans[kNMax], S[kNMax], f[kNMax];

int fetch(int x) {
	int res = lower_bound(dis,dis+nDis,x)-dis;
	if(!(dis[res]==x))
		assert(0);
	return res;
}
...
int main() {
	...
	build(1, 0, nDis-1);
	for ( int i = n; i >= 1; i -- ) {
		int k = min(askMin(g+s[i]-p, s[i]-1), askMin(g+s[i], p-1));
		if ( k == kInf ) 
			f[i] = S[n+1]-S[i];
		else
	...
	for ( int i = 1; i <= q; i ++ ) {
		int k = min(askMin(g-t[i]%p,p-t[i]%p-1), askMin(g+p-t[i]%p, p-1));
		if ( k == kInf )
			ans[i] = t[i]+S[n+1];
		else
