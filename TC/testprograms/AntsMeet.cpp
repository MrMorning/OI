		...
		int countAnts(vector <int> x, vector <int> y, string direction) {
			...
			while ( 1 ) {
				...
				for ( int i = 0; i < n; i ++ )
					for ( int j = i + 1; j < n; j ++ )
						if ( alive[i] && alive[j] ) {
							int a1 = kDx[dir[i]], b1 = kDy[dir[i]];
							int a2 = -kDx[dir[j]], b2 = -kDy[dir[j]];
							int a3 = x[j] - x[i], b3 = y[j] - y[i];
							if ( a1 + a2 == 0 && a3 != 0 ) continue;
							if ( b1 + b2 == 0 && b3 != 0 ) continue;
							if ( a1 + a2 == 0 && (double)b3 / (b1 + b2) >= 0 ) {
								assert(b1 + b2 != 0);
								death[i] = min(death[i], (double)b3 / (b1 + b2));
								death[j] = min(death[j], (double)b3 / (b1 + b2));
							}
							if ( b1 + b2 == 0 && (double)a3 / (a1 + a2) >= 0 ) {
								assert(a1 + a2 != 0);
								death[i] = min(death[i], (double)a3 / (a1 + a2));
								death[j] = min(death[j], (double)a3 / (a1 + a2));
							}
							if ( a3 * (b1 + b2) != b3 * (a1 + a2) ) continue;
							if ( (double)a3 / (a1 + a2) >= 0 ) {
								death[i] = min(death[i], (double)a3 / (a1 + a2));
								death[j] = min(death[j], (double)a3 / (a1 + a2));
							}
						}
				...
				for ( int i = 0; i < n; i ++ )
					if ( alive[i] )
						minTime = min(minTime, death[i]);
				if ( minTime == 1e100 )
					break;
				for ( int i = 0; i < n; i ++ )
					if ( alive[i] && fabs(minTime - death[i]) < EPS ) 
						alive[i] = false;
			}
			int ans = 0;
			for ( int i = 0; i < n; i ++ )
