...
} pw[kNMax], pb[kNMax], dir[kNMax], f[kNMax], core;
int nw, nb;
bool mark[kNMax];
...
bool cmp(const Point &p1, const Point &p2) {
	return cross(p1-core, p2-core) > 0;
}
...
public:
	string isFree(vector <string> whiteX, vector <string> whiteY, vector <string> blackX, vector <string> blackY) {
		...
		stringstream ss3;
		ss3 << bXstr;
		while ( ss3 >> pb[nb].x ) nb++;
		stringstream ss4;
		ss4 << bYstr;
		for ( int i = 0; i < nb; i ++ )
			ss4 >> pb[i].y;
		if ( nb <= 1 || nw <= 1 ) return "YES";
		for ( int i = 0; i < nb; i ++ )
			for ( int j = i+1; j < nb; j ++ ) {
				int nd=0;
				Point p1 = pb[i], p2 = pb[j];
				memset(mark, 0, sizeof(mark));
				for ( int k = 0; k < nw; k ++ )
					if ( cross(p1-p2, pw[k]-p2) > 0 ) 
						mark[k] = true;
					else 
						dir[nd++] = pw[k];
				if ( nd ) {
					...
					for ( int k = 0; k < nd; k ++ ) {
						f[k] = dir[k];
						if ( k > 0 && cross(f[k]-p2, f[k-1]-p2) > 0 ) f[k] = f[k-1];
					}
					for ( int k = 0; k < nw; k ++ ) 
						if ( mark[k] ) {
							int lb = 0, rb = nd;
							for ( int mid = (lb + rb) / 2; lb < rb - 1; mid = (lb + rb) / 2 )
								if ( cross(p1-pw[k], dir[mid]-p1) < 0 )
									lb = mid;
								else rb = mid;
							if ( cross(p1-pw[k], dir[lb]-p1) < 0 )
								if ( cross(p2-pw[k], f[lb]-p2) > 0 )
									return "NO";
						}
				}
			}
...
int main() {
	...
	{
		// ----- test 0 -----
		string t0[] = {"8583129 188354 8561147 179907 184107 8572156 62437", "36 6228516 6258944"};
		p0.assign(t0, t0 + sizeof(t0) / sizeof(t0[0]));
		string t1[] = {"1512711 3640563 1490266 3670822 3655686 1501471 98", "42842 9846725 9838910"};
		p1.assign(t1, t1 + sizeof(t1) / sizeof(t1[0]));
		string t2[] = {"3953706 6646660 4389875 3957787 3961878 6647461 43", "99632 6648246 4394751"};
		p2.assign(t2, t2 + sizeof(t2) / sizeof(t2[0]));
		string t3[] = {"3702677 4742544 6550975 3699397 3696129 4747718 65", "54777 4752895 6552884"};
