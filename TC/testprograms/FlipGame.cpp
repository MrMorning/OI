...
class FlipGame {
	public:
		vector<string> board;
		int ptr[55];
		int prevOne(int i, int j, int cnt) {
			for ( int k = j; k >= 0; k -- )
				if ( ((board[i][k]-'0')^cnt) == 1 )
					return k;
			return -1;
		}
		int minOperations(vector <string> _board) {
			board = _board;
			int n = board.size(), m = board[0].size();
			int ans = 0;
			for ( int i = 0; i < n; i ++ )
				ptr[i] = prevOne(i, m - 1, 0);
			while ( 1 ) {
				bool suc = true;
				for ( int i = 0; i < n; i ++ )
					if ( ptr[i] != -1 )
				...
				for ( int i = 0; i < n; i ++ )
					if ( ptr[i] != -1 ) {
						int cur = ptr[i];
						ptr[i] = prevOne(i, ptr[i], (ans&1)^1);
						for ( int j = i + 1; j < n; j ++ )
							if ( ptr[j] < cur ) 
								ptr[j] = cur;
							else {
								cur = ptr[j];
								ptr[j] = prevOne(j, cur, (ans&1)^1);
							}
						ans++;
						break;
					}
