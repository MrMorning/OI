...
int main()
{
	...
	for(int i = 0; i <= sum[n]; i ++)
		memset(f[0][i], 0x3f, sizeof(int) * (sum[n] + 1));
	f[0][0][0] = 0;
	...
	for(int i = 0; i < n; i ++)
	{
		...
		for(int t = 0; t <= sum[n]; t ++)
			memset(f[1][t], 0x3f, sizeof(int) * (sum[n] + 1));
		//for(int t = 0; t <= sum[n]; t ++)
		//	memset(marked, 0, sizeof(bool) * (sum[n] + 1));
		tail[! now] = 0;
		...
		for(int t = 0; t <= sum[n]; t ++)
			memcpy(f[0][t], f[1][t], sizeof(int) * (sum[n] + 1));
		now = ! now;
	}
	int ans = INF;
