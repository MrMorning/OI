...
void Solve::solve(FILE *fin, FILE *fout)
{
	...
	floyd();
	Val_d static arr_t[200 * 200 + 1];
	Val_d static arr_bin[200 * 200 + 1];
	int nArr_t = 0, nArr_bin = 0;
	for(int u = 0; u < nVtx; u ++)
		for(int v = 0; v < nVtx; v ++)
			arr_t[nArr_t ++] = dist[u][v];
	arr_t[nArr_t] = INF_D;
	std::sort(arr_t, arr_t + nArr_t);
	for(int i = 0; i < nArr_t; i ++)
		if(arr_t[i] != arr_t[i + 1])
			arr_bin[nArr_bin ++] = arr_t[i];
	arr_bin[nArr_bin] = INF_D;
	Val_d l = 0, r = nArr_bin, mid = (l + r) / 2;
	while(l < r)
	{
		SaveAndLoad::load();
		if(check(arr_bin[mid]))
			r = mid;
		else
			l = mid + 1;
		mid = (l + r) / 2;
	}
	if(arr_bin[mid] == INF_D)
		mid = -1;
