...
namespace solve1 {
	...
	void main(vector<pair<int, int> > &res1, vector<pair<int, int> > &res2, int &nPart) {
		...
		for ( int i = 1; i <= nWolf; i ++ ) 
			if ( !wolf[i].del ) {
				int lb = lower_bound(agl, agl+nAgl, calcAgl(wolf[i].p1))-agl,
					rb = lower_bound(agl, agl+nAgl, calcAgl(wolf[i].p2))-agl;
				res1.pb(mp(2*lb, 2*rb));
			}
		for ( int i = 1; i <= nSheep; i ++ ) {
			int lb = lower_bound(agl, agl+nAgl, calcAgl(sheep[i].p1))-agl,
				rb = lower_bound(agl, agl+nAgl, calcAgl(sheep[i].p2))-agl;
