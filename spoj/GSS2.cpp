...
struct Stuff
{
	...
	bool e,marked;
	Stuff(bool c=true):e(c){maxs=maxnow=tag=mtag=0;marked=false;}
...
void mark(int t,int x)
{
	T[t].maxnow+=x;
	T[t].tag+=x;
	T[t].mtag=max(T[t].mtag,T[t].tag);
	T[t].maxs=max(T[t].maxs,T[t].maxnow);
	T[t].marked=true;
}
...
void pushdown(int t)
{
	if(T[t].marked)
	{
		...
		T[2*t+1].tag+=T[t].tag;
		T[2*t+1].maxs=max(T[2*t+1].maxs,T[2*t+1].maxnow+T[t].mtag);
