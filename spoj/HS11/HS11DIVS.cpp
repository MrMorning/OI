...
inline void increase(ll &a, ll b)
{
	a = MOD(a + b);
}
...
ll solve(const BigInt &n, int x, int l, int r)
{
	...
	for(int i = n.len - 1; i >= 0; i --)
	{
		...
		sumOfDigits += n.num[i];
		modOfDigits = (modOfDigits * 10 + n.num[i]) % x;
	}
	return res;
}
...
int main()
{
	...
	pow[0] = 1 % x;
	for(int i = 1; i <= 100; i ++)
		pow[i] = (ll)pow[i - 1] * 10 % x;
	init(x);
	printf("%d\n", (int)MOD(solve(b, x, l, r) - solve(a, x, l, r)));
}
