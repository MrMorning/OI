...
void calc_g(int x, int d, double val, pair<double, int> *g) {
...
void work(int x) {
	...
	static pair<double, int> f[V_MAX], g[V_MAX];
	...
	static MonoQueue<pair<double, int> > MQ;
	...
	foreach(it, son) {
		...
		for ( int l1 = it->first; l1 >= 0; l1 -- )
			if ( g[l1].first != -inf ) {
				...
				if ( !MQ.size() )
					continue;
				pair<double, int> tp = MQ.get_max();
				if ( tp.first == -inf )
					continue;
				if ( tp.first + g[l1].first > ans ) {
					ans = tp.first + g[l1].first;
				}
			}
