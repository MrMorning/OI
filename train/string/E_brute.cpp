...
char data[4200010];
char *str[2100010], palind[2100010]; int len[2100010];
int kmp[2100010], trie[4200010][2], triemr;

typedef struct { char letter; int son, next; } graphnode;
graphnode graph[8400010]; int grapmr;

// 程序中所有带 d 的函数，所做的都是相反的事情。也就是正向匹配完成之后要逆向匹配。
...
int main ()
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        for (j = len[i] - 1; j >= 0; j--)
        {
            while (k > -1 && str[i][j] != str[i][k]) k = kmp[k];
            k++;
        }
        while (k > 0)
        {
