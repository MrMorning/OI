...
struct Block {
	int l, r, sz;
	int sorted[BLK_SIZE], f[N_MAX], g[N_MAX], less[N_MAX];
} blk[BLK_MAX];
int n_blk;
...
void init_block() {
	...
	for ( int i = 0; i < n_blk; i ++ ) {
		...
		for ( int j = 0; j < blk[i].sz; j ++ ) {
			for ( int k = pre_id; k <= blk[i].sorted[j]; k ++ )
				blk[i].less[k] = pre_val;
			int st = j;
			while ( j + 1 < blk[i].sz && blk[i].sorted[j] == blk[i].sorted[j + 1]) 
				j ++;
			pre_id = blk[i].sorted[j] + 1;
			pre_val += j - st + 1;
		}

		for ( int k = pre_id; k < axis_cnt; k ++ )
			blk[i].less[k] = pre_val;
	}
