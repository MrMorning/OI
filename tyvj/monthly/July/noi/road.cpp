...
namespace sgt
{
	...
	Node *build(int l, int r, int buf[])
	{
		...
		if(l == r)
		{
			x->val = weight[buf[depth[buf[0]] - l]];
			x->ch[0] = x->ch[1] = NULL;
		}
		else
		{
			int mid = (l + r) / 2;
			x->ch[0] = build(l, mid, buf);
			x->ch[1] = build(mid + 1, r, buf);
...
void chainSplit()
{
	...
	for(int i = seqCnt - 1; i >= 0; i --)
	{
		...
		sgt::Node *x = sgt::build(depth[buf[bufCnt - 1]], depth[buf[0]], buf);
...
int letItJump(int u, int p)
{
	...
	for(; root[u] != root[p]; u = jump[u])
		result = max(result, sgt::ask(root[u], 0, depth[u]));
	if(depth[p] < depth[u])
		result = max(result, sgt::ask(root[u], depth[p] + 1, depth[u]));
	return result;
}
